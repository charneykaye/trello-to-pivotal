...
module.exports = {
     ...
     */
    checklistItemsFromTrello(card, checklists) {
        let items = [];
        for (let id in checklists) {
            if (checklists[id]['idCard'] === card['id']) {
                for (let n = 0; n < checklists[id]['checkItems'].length; n++) {
                    items.push(checklists[id]['checkItems'][n]);
                }
            }
        }
        return items;
